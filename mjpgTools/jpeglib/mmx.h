// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////// //
// mmx.h:	part of the mjpgTools Library																				 //
...
typedef	union {
	...
	INT32				d[2];	/* 2 Doubleword (32-bit) values */
	UINT32				ud[2];	/* 2 Unsigned Doubleword */
	INT16				w[4];	/* 4 Word (16-bit) values */
	UINT16				uw[4];	/* 4 Unsigned Word */
	INT8				b[8];	/* 8 Byte (8-bit) values */
	UINT8				ub[8];	/* 8 Unsigned Byte */
	float				s[2];	/* Single-precision (32-bit) value */
} ATTR_ALIGN(8) mmx_t;	/* On an 8-byte (64-bit) boundary */


#define	mmx_si2r(op,imm,reg) \
	__asm volatile (#op " %0, %%" #reg \
			      : /* nothing */ \
			      : "J" (imm) )
...
#define	mmx_r2m(op,reg,mem) \
	__asm volatile (#op " %%" #reg ", %0" \
			      : "=X" (mem) \
			      : /* nothing */ )

#define	mmx_r2r(op,regs,regd) \
	__asm volatile (#op " %" #regs ", %" #regd)


#define	emms() __asm volatile ("emms")
#define	femms() __asm volatile ("femms")
...
#define mmx_m2ri(op,mem,reg,imm) \
        __asm volatile (#op " %1, %0, %%" #reg \
                              : /* nothing */ \
                              : "X" (mem), "X" (imm))
#define mmx_r2ri(op,regs,regd,imm) \
        __asm volatile (#op " %0, %%" #regs ", %%" #regd \
                              : /* nothing */ \
                              : "X" (imm) )

#define	mmx_fetch(mem,hint) \
	__asm volatile ("prefetch" #hint " %0" \
			      : /* nothing */ \
			      : "X" (mem))
...
#define	pmovmskb(mmreg,reg) \
	__asm volatile ("movmskps %" #mmreg ", %" #reg)
