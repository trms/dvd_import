// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////// //
// ratectl.c: bitrate control routines (linear quantization only currently), part of the mjpgTools Library				 //
// --------------------------------------------------------------------------------------------------------------------- //
...
static int d;
static int32_t T;
static int64_t bits_transported = 0;
static int64_t bits_used = 0;
...
void rc_init_seq(int reinit)
{
	...
	*/
	...
	{
		per_pict_bits = (int32_t)(opt_fieldpic ? opt_bit_rate / field_rate : opt_bit_rate / ctl_decode_frame_rate);
		R = (int32_t)opt_bit_rate;
	}
	...
	*/
	...
	else
	{
		...
		if( undershoot_carry < 0 )
			errorExit_msg("Rate control can't cope with a video buffer smaller 4 frame intervals");
	...
	*/
	...
	d0p = d0b = d0pb = d0i = (int)(init_quant * r / 62.0);
...
void rc_init_GOP( int np, int nb)
{
	...
	*/
	...
	else
	{
		...
		int available_bits = (int)( ( opt_bit_rate+buffer_variation*recovery_gain)*fields_in_gop/field_rate );
		double Xsum = Ni*Xi+Np*Xp+Nb*Xb;
		debug_message( "REST GOP INIT" );
...
void rc_init_pict(pict_data_s *picture)
{
	...
	*/
	...
	{
		int feedback_correction = (int)( fast_tune ? buffer_variation * overshoot_gain : (buffer_variation+gop_buffer_correction) * overshoot_gain );
		available_bits = (int)((opt_bit_rate+feedback_correction)*fields_in_gop/field_rate);
	}
	...
	*/
	...
	debug_message("I=%d P=%d B=%d T=%05d A=%06d D=%06d (%06d)",
	...
	*/
	...
	debug_message("PBB=%d PPB=%d", pict_base_bits, per_pict_bits);
	...
	*/
	...
	if( opt_still_size > 0 && opt_vbv_buffer_still_size )
	{
		/* If stills size must match then target low to ensure no
		   overshoot.
		*/
		info_message( "Setting VCD HR still overshoot margin to %d bytes", T/(16*8) );
...
void rc_update_pict(pict_data_s *picture)
{
	...
	int    Qsum;
	int frame_overshoot;
	static int64_t prev_bitcount = 0;
	mpg2enc_msgTitle = "rc_update_pict";

	AP = (int32_t)(bitcount() - S);
	...
	*/
	...
	if( opt_still_size > 0 && opt_vbv_buffer_still_size)
	{
		...
		{
			warn_message( "Rate overshoot: VCD hi-res still %d bytes too large! ", 
		...
		if( padding_bytes > 0 )
		{
			debug_message( "Padding still to size: %d extra bytes", padding_bytes );
			picture->pad = 1;
			alignbits();
	...
	*/
	...
	debug_message( "TR=%ld USD=%ld", bits_transported/8, bits_used/8);
...
int rc_start_mb(pict_data_s *picture)
{
	...
	mquant = intmax(mquant, (int)ctl_quant_floor);
...
void calc_vbv_delay(pict_data_s *picture)
{
	...
	mpg2enc_msgTitle = "calc_vbv_delay";
	...
	*/
	if (!opt_low_delay && (decoding_time < (double)bitcnt_EOP*90000.0/opt_bit_rate))
	{
		...
		warn_message("vbv_delay underflow frame %d (target=%.1f, actual=%.1f)",
		...
		> vbv_buffer_size )
	{
		...
		if(!quiet || oversize > 0.0  )
			warn_message("vbv_delay overflow frame %d - %f.0 bytes!", 
					   frame_num,
					   oversize / 8.0
				);
	}
	...
	{
		warn_message("vbv_delay underflow: %d",picture->vbv_delay);
		picture->vbv_delay = 0;
	}
	...
	{
		warn_message("vbv_delay frame %d exceeds permissible range: %d",
	...
	if( !opt_mpeg1 || ctl_quant_floor != 0 || opt_still_size > 0)
		...
		picture->vbv_delay =  (int)(90000.0/opt_frame_rate/4);
#endif
