...
extern uint8_t				mpg2enc_verbose;		// 0=silent, 1=info, 2=debug
extern const char*			mpg2enc_msgTitle;		// NULL or a custom title, set by a routine
extern const char*			mpg2enc_defTitle;		// "" : usually a name that identifies the class
...
void	errorExit(void); // will raise an exception, so we realy break out operation
void	errorExit_msg(const char* lpFilter, ...);	// Raises an exception after an error-message has been showed
