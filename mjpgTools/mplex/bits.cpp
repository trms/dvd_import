// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////// //
// bits.cpp: implementation of the BitStreamUndo class, part of the mjpgTools Library									 //
//			 implementation of the BitStream class, part of the mjpgTools Library										 //
//			 implementation of the IBitStream class, part of the mjpgTools Library										 //
//			 implementation of the OBitStream class, part of the mjpgTools Library										 //
// --------------------------------------------------------------------------------------------------------------------- //
...
//	 Additional portions and modifications - Copyright (C) 2001 Andrew Stevens	<as@comlab.ox.ac.uk>					 //
//																														 //
...
//	 including any implied warranties or merchantability or of fitness for a particular purpose.  In no event shall the	 //
//	 copyright-holder be liable for any incidental, punitive, or consequential damages of any kind whatsoever arising	 //
//	 from the use of these programs.																					 //
//	 This disclaimer of warranty extends to the user of these programs and user's customers, employees, agents,			 //
//	 transferees, successors, and assigns																				 //
//	 The MPEG Software Simulation Group does not represent or warrant that the programs furnished hereunder are free of	 //
//	 infringement of any third-party patents.																			 //
//	 Commercial implementations of MPEG-1 and MPEG-2 video, including shareware, are subject to royalty fees to patent	 //
//	 holders. Many of these patents are general enough such that they are unavoidable regardless of implementation design./
// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////// //
//#include <config.h>
#include <stdlib.h>
#include <errno.h>
...
const unsigned int BitStream::BUFFER_SIZE = 512 * 1024;
/* initialize buffer, call once before first putbits or alignbits */
void OBitStream::open(char *bs_filename, unsigned int buf_size)
{
	msgTitle = "OBitStream::open";
	if ((fileh = fopen(bs_filename, "wb")) == NULL)
	{
		error_exit_msg( "Unable to open file %s for writing; %s", 
...
{
	msgTitle = "OBitStream::putbyte";
    bfr[byteidx++] = outbyte;
    if (byteidx == bfr_size)
    {
		if (fwrite(bfr, sizeof(unsigned char), bfr_size, fileh) != bfr_size)
			error_exit_msg( "Write failed: %s", strerror(errno));
		byteidx = 0;
    }
	bitidx = 8;
}
...
{
	msgTitle = "IBitStream::refill_buffer";
	size_t i;
	if( bufcount >= bfr_size )
	{
		error_exit_msg("INTERNAL ERROR: additional data required but "
...
{
	msgTitle = "IBitStream::flush";
	if( flush_upto > buffer_start+bufcount )
		error_exit_msg("INTERNAL ERROR: attempt to flush input beyond buffered amount" );

	if( flush_upto < buffer_start )
		error_exit_msg("INTERNAL ERROR: attempt to flush input stream before  first buffered byte %d last is %d", flush_upto, buffer_start );
...
{
	msgTitle = "IBitStream::read_buffered_bytes";
	unsigned int to_read = length;
	if( readpos < buffer_start)
		error_exit_msg("INTERNAL ERROR: access to input stream buffer @ %d: before first buffered byte (%d)", readpos, buffer_start );
    debug_Message( "Writing to %ld",  readpos+length );
	if( readpos+length > buffer_start+bufcount )
	{
		if( !feof(fileh) )
        {
			error_exit_msg("INTERNAL ERROR: access to input stream buffer beyond last buffered byte @POS=%ld END=%d REQ=%ld + %d bytes", 
                        readpos,
...
{
	msgTitle = "IBitStream::open";

	if ((fileh = fopen(bs_filename, "rb")) == NULL)
	{
		error_exit_msg( "Unable to open file %s for reading.", bs_filename);
	}
	filename = strcpy( new char[strlen(bs_filename)+1], bs_filename );
	...
	if (!refill_buffer())
	{
		if (bufcount==0)
		{
			error_exit_msg( "Unable to read from file %s.", bs_filename);
		}
	}
}
