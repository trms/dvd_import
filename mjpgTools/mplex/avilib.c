// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////// //
// avilib.c:	part of the mjpgTools Library																			 //
//				Some utilities for writing and reading AVI files. These are not intended to serve for a full blown AVI	 //
//				handling software (this would be much too complex)														 //
//				The only intention is to write out MJPEG encoded AVIs with sound and to be able to read them back again. //
...
static INT32 avi_write_data(avi_t* AVI, UINT8* data, INT32 length, INT32 audio)
{
	...
	if(audio)
		...
		n = avi_add_index_entry(AVI,(const UINT8*)"00db",0x10,AVI->pos,length);
	...
	if(audio)
		...
		n = avi_add_chunk(AVI,(const UINT8*)"00db",data,length);
...
INT32 AVI_dup_frame(avi_t* AVI)
{
	...
	if(avi_add_index_entry(AVI,(const UINT8*)"00db",0x10,AVI->last_pos,AVI->last_len)) return -1;
	AVI->video_frames++;
