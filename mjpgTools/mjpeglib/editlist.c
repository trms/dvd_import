// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////// //
// editlist.c:	part of the mjpgTools Library																			 //
// --------------------------------------------------------------------------------------------------------------------- //
...
void read_video_files(char** filename, INT32 num_files, EditList* el, INT32 preserve_pathnames)
{
   FILE* fd;
   char line[1024];
   INT32 index_list[MAX_EDIT_LIST_FILES];
   INT32 i, n, nf, n1, n2, nl, num_list_files;
   ...
   for(;nf<num_files;nf++)
   {
      ...
      if(strcmp(line,"LAV Edit List\n")==0)
      {
         ...
         while(fgets(line,1024,fd))
         {
            if(line[0]!=':') /* ignore lines starting with a : */
            {
               ...
               el->frame_list = (INT32*) realloc(el->frame_list,
                                   (el->video_frames+n2-n1+1)*sizeof(INT32));
      ...
      else
      {
         ...
         el->frame_list = (INT32*) realloc(el->frame_list,
                             (el->video_frames+el->num_frames[n])*sizeof(INT32));
         if(el->frame_list==0) malloc_error();
...
{
   FILE* fd;
   INT32 i, n, num_files, oldfile, oldframe;
   INT32 index[MAX_EDIT_LIST_FILES];
...
INT32 el_get_audio_data(UINT8* abuff, INT32 nframe, EditList* el, INT32 mute)
{
   ...
   ns1 = (INT32)((double)(N_EL_FRAME(n)+1)*el->audio_rate/el->video_fps);
   ns0 = (INT32)((double) N_EL_FRAME(n)   *el->audio_rate/el->video_fps);
...
INT32 el_video_frame_data_format(INT32 nframe, EditList* el)
{
   ...
   if      (strnicmp(comp,"yv12",4)==0) return DATAFORMAT_YUV420;
   else if (strnicmp(comp,"yuv2",4)==0) return DATAFORMAT_YUV422;
   else if (strnicmp(comp,"dv",2)==0) return DATAFORMAT_DV2;
   else if (strnicmp(comp,"mjp",3)==0 || strnicmp(comp,"jpeg",4)==0)
