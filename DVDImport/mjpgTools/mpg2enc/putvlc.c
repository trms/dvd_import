// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////// //
// putvlc.c: generation of variable length codes, part of the mjpgTools Library											 //
// --------------------------------------------------------------------------------------------------------------------- //
...
{
  int absval, size;
	mpg2enc_msgTitle = "putDC";

  absval = abs(val);
  if (absval>opt_dctsatlim)
  {
    /* should never happen */
    errorExit_msg("Internal: DC value out of range (%d)",val);
  }
...
int run,signed_level,vlcformat;
{
  ...
  if (run<0 || run>63 || level==0 || level>opt_dctsatlim)
  {
	  ...
	  {
		  errorExit_msg("Internal: AC value out of range (run=%d, signed_level=%d)", run,signed_level);
	  }
  }
