// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////// //
// putseq.c, sequence level routines, part of the mjpgTools Library														 //
// --------------------------------------------------------------------------------------------------------------------- //
							...
							int min_b_grp)
{
    ...
    */
	...
	int pred_lum_mean;
	mpg2enc_msgTitle = "find_gop_length";
        ...
        if( gop_len == 0 )
        {
            ...
            */
                ...
                && gop_len < gop_max_len+gop_min_len )
            {
                ...
                {
                    info_message("GOP min length too small to permit scene-change on GOP boundary %d", j);
	...
	info_message( "GOP LENGTH = %d", gop_len);
	return gop_len;
...
static void gop_start( stream_state_s *ss )
{
    ...
    */
	...
	if( (ss->next_split_point != 0 && bits_after_mux > (uint64_t)ss->next_split_point)
		|| (ss->i != 0 && opt_seq_end_every_gop)
		)
	{
		info_message( "Splitting sequence this GOP start" );
	...
	*/
	...
	{
		errorExit_msg( "****INTERNAL: inconsistent GOP %d %d %d", 
						   ss->gop_length, np, nb);
	}
...
{
	mpg2enc_msgTitle = "stencodeworker";
	/* ALWAYS do-able */
	info_message("Frame start %d %c %d %d",
			   picture->decode, 
			   pict_type_char[picture->pict_type],
			   picture->temp_ref,
			   picture->present);
	...
	*/
	...
	info_message("Frame end %d %s %3.2f %.2f %2.1f %.2f",
				picture->decode, 
				picture->pad ? "PAD" : "   ",
...
//	pthread_t worker_threads[MAX_WORKER_THREADS];
	...
	*/
	...
	debug_message( "Split len = %ld", ss.seq_split_length );
...
//		sync_guard_update( &cur_picture->completion, 0 );
		...
		if( readframe(cur_picture->temp_ref+ss.gop_start_frame,cur_picture->curorg) )
		    errorExit_msg("Corrupt frame data in frame %d aborting!", cur_picture->temp_ref+ss.gop_start_frame );


		set_pic_params( ss.i, ss.b,   cur_picture );

		stencodeworker( cur_picture ); // do it
...
void putseq2(const void* o_hWnd, const unsigned o_uWnd)
{
	stream_state_s	ss;
	int				cur_ref_idx = 0;
	int				cur_b_idx = 0;
	...
	*/
	...
	while( frame_num<istrm_nframes )
	{
		if ((o_hWnd) && (o_uWnd))
		{
			sprintf(prgText, "Encoding frame #%ld", frame_num+1);
			lhkProgress_Step((const void*)o_hWnd, (const unsigned)o_uWnd, prgText, (const long)frame_num+1);
		}
