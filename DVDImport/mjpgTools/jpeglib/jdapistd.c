// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////// //
// jdapistd.c:	part of the mjpgTools Library																			 //
//				This file contains application interface code for the decompression half of the JPEG library.			 //
//				These are the "standard" API routines that are used in the normal full-decompression case. They are not	 //
//				used by a transcoding-only application. Note that if an application links in jpeg_start_decompress, it	 //
//				will end up linking in the entire decompressor.															 //
//				We thus must separate this file from jdapimin.c to avoid linking the whole decompression library into a	 //
//				transcoder.																								 //
// --------------------------------------------------------------------------------------------------------------------- //
...
output_pass_setup (j_decompress_ptr cinfo)
{
  ...
  while (cinfo->master->is_dummy_pass) {
    ...
    while (cinfo->output_scanline < cinfo->output_height) {
      ...
      /* Call progress monitor hook if present */
      if (cinfo->progress != NULL) {
	cinfo->progress->pass_counter = (INT32) cinfo->output_scanline;
	cinfo->progress->pass_limit = (INT32) cinfo->output_height;
	(*cinfo->progress->progress_monitor) ((j_common_ptr) cinfo);
      }
      /* Process some data */
...
GLOBAL(boolean)
jpeg_start_output (j_decompress_ptr cinfo, INT32 scan_number)
{
