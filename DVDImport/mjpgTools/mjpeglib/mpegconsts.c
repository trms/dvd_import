// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////// //
// mpegconsts.c:	part of the mjpgTools Library																		 //
//					Video format constants for MPEG and utilities for display and conversion to format used for yuv4mpeg //
// --------------------------------------------------------------------------------------------------------------------- //
...
y4m_ratio_t mpeg_conform_framerate( double fps )
{
	...
	for (i = 1; i < mpeg_num_framerates; i++) 
	{
		deviation = 1.0 - (Y4M_RATIO_DBL(mpeg_framerates[i]) / fps);
	...
	result.n = (INT32)(fps * 1000000.0 + 0.5);
	result.d = 1000000;
...
y4m_ratio_t mpeg_aspect_ratio( INT32 mpeg_version,  mpeg_aspect_code_t code )
{
	y4m_ratio_t ratio;
...
mpeg_aspect_code_t mpeg_frame_aspect_code( INT32 mpeg_version, y4m_ratio_t aspect_ratio )
{
	mpeg_aspect_code_t i;
	y4m_ratio_t red_entry;
	...
	{
		red_entry =  mpeg_aspect_ratios[mpeg_version-1][i-1];
...
mpeg_aspect_code_t mpeg_guess_mpeg_aspect_code(INT32 mpeg_version, y4m_ratio_t sampleaspect, INT32 frame_width, INT32 frame_height)
{
	...
	switch (mpeg_version)
	{
		...
		case 2 :
		{
			...
			{
				INT32 i;
				/* true frame aspect ratio */
				double true_far = (double)(sampleaspect.n * frame_width) / (double)(sampleaspect.d * frame_height);
				/* start at '2'... */
				double ratio;
				for (i = 2; i < (INT32)(mpeg_num_aspect_ratios[mpeg_version-1]); i++) 
				{
					ratio = true_far / Y4M_RATIO_DBL(mpeg_aspect_ratios[mpeg_version-1][i-1]);
 ...
 *
 */
y4m_ratio_t mpeg_guess_sample_aspect_ratio(INT32 mpeg_version, mpeg_aspect_code_t code, INT32 frame_width, INT32 frame_height)
{
	switch (mpeg_version) 
	{
		case 1 :
...
const char* mpeg_aspect_code_definition( INT32 mpeg_version, mpeg_aspect_code_t code )
{
	if( mpeg_version < 1 || mpeg_version > 2 )
...
const char* mpeg_interlace_code_definition( INT32 yuv4m_interlace_code )
{
	const char* def;
