// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////// //
// yuv4mpeg.c:	part of the mjpgTools Library																			 //
...
static INT32 _y4mparam_allow_unknown_tags = 1;  /* default is forgiveness */

static void *(*_y4m_alloc)(size_t bytes) = malloc;
static void (*_y4m_free)(void *ptr) = free;


INT32 y4m_allow_unknown_tags(INT32 yn)
{
	INT32 old = _y4mparam_allow_unknown_tags;
	if (yn >= 0)
...
ssize_t y4m_read(INT32 fd, void* buf, size_t len)
{
	ssize_t n;
	UINT8* ptr = (UINT8*)buf;
...
ssize_t y4m_write(INT32 fd, const void* buf, size_t len)
{
	ssize_t n;
	...
	{
		n = _write(fd, ptr, len);
		if (n <= 0) return len-n;  /* return amount left to write */
		ptr += n;
		len -= n;
	}
	return 0;
...
static INT32 y4m_snprint_xtags(char* s, INT32 maxn, const y4m_xtag_list_t* xtags)
{
	INT32 i, room, n;

	for (i = 0, room = maxn - 1; i < xtags->count; i++)
	{
		n = _snprintf(s, room + 1, " %s", xtags->tags[i]);
...
INT32 y4m_xtag_count(const y4m_xtag_list_t* xtags)
{
	return xtags->count;
}

const char* y4m_xtag_get(const y4m_xtag_list_t* xtags, INT32 n)
{
	if (n >= xtags->count)
		return NULL;
	else
...
INT32 y4m_xtag_add(y4m_xtag_list_t* xtags, const char* tag)
{
...
INT32 y4m_xtag_remove(y4m_xtag_list_t* xtags, INT32 n)
{
	INT32 i;
	char* q;
...
INT32 y4m_xtag_clearlist(y4m_xtag_list_t* xtags)
{
	xtags->count = 0;
	return Y4M_OK;
}

INT32 y4m_xtag_addlist(y4m_xtag_list_t* dest, const y4m_xtag_list_t* src)
{
	INT32 i, j;
...
void y4m_copy_stream_info(y4m_stream_info_t* dest, const y4m_stream_info_t* src)
{
...
void y4m_si_set_width(y4m_stream_info_t* si, INT32 width)
{
	si->width = width;
	si->framelength = (si->height * si->width) * 3 / 2;
}

INT32 y4m_si_get_width(const y4m_stream_info_t* si)
{
	return si->width;
}

void y4m_si_set_height(y4m_stream_info_t* si, INT32 height)
{
	si->height = height; 
	si->framelength = (si->height * si->width) * 3 / 2;
}

INT32 y4m_si_get_height(const y4m_stream_info_t* si)
{
	return si->height;
}

void y4m_si_set_interlace(y4m_stream_info_t* si, INT32 interlace)
{
	si->interlace = interlace;
}

INT32 y4m_si_get_interlace(const y4m_stream_info_t* si)
{
	return si->interlace;
}
...
INT32 y4m_si_get_framelength(const y4m_stream_info_t* si)
{
...
INT32 y4m_parse_stream_tags(char* s, y4m_stream_info_t* i)
{
	char* token, *value;
	char tag;
	INT32 err;
...
static INT32 y4m_parse_frame_tags(char* s, y4m_frame_info_t* i)
{
	char* token, *value;
	char tag;
	INT32 err;
...
INT32 y4m_read_stream_header(INT32 fd, y4m_stream_info_t* i)
{
...
INT32 y4m_write_stream_header(INT32 fd, const y4m_stream_info_t* i)
{
	...
	n = _snprintf(s, sizeof(s), "%s W%d H%d F%d:%d I%s A%d:%d", Y4M_MAGIC, i->width, i->height, rate.n, rate.d,
		(i->interlace == Y4M_ILACE_NONE) ? "p" : (i->interlace == Y4M_ILACE_TOP_FIRST) ? "t" : \
		(i->interlace == Y4M_ILACE_BOTTOM_FIRST) ? "b" : "?", aspect.n, aspect.d);
	if ((n < 0) || (n > Y4M_LINE_MAX)) return Y4M_ERR_HEADER;
	if ((err = y4m_snprint_xtags(s + n, sizeof(s) - n - 1, &(i->x_tags))) != Y4M_OK) 
		return err;
	/* non-zero on error */
...
INT32 y4m_read_frame_header(INT32 fd, y4m_frame_info_t* i)
{
	char line[Y4M_LINE_MAX];
	char* p;
	INT32 n;
...
INT32 y4m_write_frame_header(INT32 fd, const y4m_frame_info_t* i)
{
	char s[Y4M_LINE_MAX+1];
	INT32 n;
	INT32 err;
...
INT32 y4m_read_frame(INT32 fd, const y4m_stream_info_t* si, y4m_frame_info_t* fi, UINT8* const yuv[3])
{
	INT32 err;
	INT32 w = si->width;
	INT32 h = si->height;
...
INT32 y4m_write_frame(INT32 fd, const y4m_stream_info_t* si, const y4m_frame_info_t* fi, UINT8* const yuv[3])
{
	INT32 err;
	INT32 w = si->width;
	INT32 h = si->height;
	...
	if ( (y4m_write(fd, yuv[0], w*h)) || (y4m_write(fd, yuv[1], w*h/4)) || (y4m_write(fd, yuv[2], w*h/4)) )
		return Y4M_ERR_SYSTEM;
	return Y4M_OK;
}
...
INT32 y4m_read_fields(INT32 fd, const y4m_stream_info_t* si, y4m_frame_info_t* fi, UINT8* const upper_field[3],
					UINT8* const lower_field[3])
{
	INT32 i, y, err;
	INT32 width = si->width;
	INT32 height = si->height;
	UINT8* srctop, *srcbot;
	...
	for (i = 0; i < 3; i++)
	{
		srctop = upper_field[i];
		srcbot = lower_field[i];
...
INT32 y4m_write_fields(INT32 fd, const y4m_stream_info_t* si, const y4m_frame_info_t* fi, UINT8* const upper_field[3],
					 UINT8* const lower_field[3])
{
	INT32 i, y, err;
	INT32 width = si->width;
...
void y4m_log_stream_info(log_level_t level, const char* prefix, const y4m_stream_info_t* i)
{
	char s[256];
	...
	if ((i->framerate.n == 0) && (i->framerate.d == 0))
		...
		mjpeg_log(level, "%s  frame rate:  %d/%d fps (~%f)", prefix, i->framerate.n, i->framerate.d,
			(double) i->framerate.n / (double) i->framerate.d);

	mjpeg_log(level, "%s   interlace:  %s", prefix, (i->interlace == Y4M_ILACE_NONE) ? "none/progressive" : \
	...
	if ((i->sampleaspect.n == 0) && (i->sampleaspect.d == 0))
		...
		mjpeg_log(level, "%ssample aspect ratio:  %d:%d", prefix, i->sampleaspect.n, i->sampleaspect.d);
}
...
const char* y4m_strerr(INT32 err)
{
	switch (err)
	{
		case Y4M_OK :
