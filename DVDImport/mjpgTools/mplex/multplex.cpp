// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////// //
// multiplex.cpp: implementation of the OutputStream class, part of the mjpgTools Library								 //
// --------------------------------------------------------------------------------------------------------------------- //
...
{
	msgTitle = "OutputStream::InitSyntaxParameters";
	...
	case MPEG_FORMAT_VCD_STILL :
		...
		{
			error_exit_msg("VCD stills has max. permissible video buffer size of 220KB");
		}
		else
		{
    ...
    case MPEG_FORMAT_DVD :
		info_Message( "Selecting DVD output profile (INCOMEPLETE!!!!)");
	...
	default : /* MPEG_FORMAT_MPEG1 - auto format MPEG1 */
		...
		always_sys_header_in_pack = (opt_always_system_headers == 0) ? false : true;
		sys_header_in_pack1 = 1;
...
void OutputStream::Init( char *multi_file)
{
	...
	info_Message("SYSTEMS/PROGRAM stream:");
	...
	*/ 
	...
	for( str = estreams->begin(); str < estreams->end(); ++str )
	{
		...
		case ElementaryStream::video :
			...
			break;
		default :
			error_exit_msg("INTERNAL: Only audio and video payload calculations implemented!");
			
		}

		if( (*str)->NominalBitRate() == 0 && opt_data_rate == 0)
			error_exit_msg( "Variable bit-rate stream present: output stream (max) data-rate *must* be specified!");
	...
	info_Message("rough-guess multiplexed stream data rate    : %07d",dmux_rate * 8);
	if( opt_data_rate != 0 )
		info_Message("target data-rate specified               : %7d", opt_data_rate*8 );

	if( opt_data_rate == 0 )
	{
		info_Message( "Setting best-guess data rate.");
	}
	else if ( opt_data_rate >= dmux_rate)
	{
		info_Message( "Setting specified specified data rate: %7d", opt_data_rate*8 );
		dmux_rate = opt_data_rate;
	}
	else if ( opt_data_rate < dmux_rate )
	{
		warn_Message( "Target data rate lower than computed requirement!\nN.b. a 20%% or so discrepancy in variable bit-rate\nstreams is common and harmless provided no time-outs will occur");
		dmux_rate = opt_data_rate;
	}
	...
	*/
	...
	info_Message( "Sectors = %d Video delay = %ld Audio delay = %ld",
				sectors_delay,
				 video_delay / 300,
				 audio_delay / 300 );
...
{
	msgTitle = "OutputStream::MuxStatus";
	using namespace std;
	vector<ElementaryStream *>::iterator str;
	for( str = estreams->begin(); str < estreams->end(); ++str )
	{
		...
		{
		case ElementaryStream::video :
			debug_Message( "Video %02x: buf=%7d frame=%06d sector=%08d",
					   (*str)->stream_id,
					   (*str)->bufmodel.Space(),
					   (*str)->au->dorder,
					   (*str)->nsec
			...
			break;
		case ElementaryStream::audio :
			debug_Message( "Audio %02x: buf=%7d frame=%06d sector=%08d",
					   (*str)->stream_id,
					   (*str)->bufmodel.Space(),
					   (*str)->au->dorder,
					   (*str)->nsec
			...
			break;
		default :
			debug_Message( "Other %02x: buf=%7d sector=%08d",
					   (*str)->stream_id,
					   (*str)->bufmodel.Space(),
					   (*str)->nsec
				);
	...
	if( !vbr )
		debug_Message( "Padding : sector=%08d",
				   pstrm.nsec
			);
}
...
{
	msgTitle = "OutputStream::OutputPrefix";
	using namespace std;
//	vector<ElementaryStream *>::iterator str;
	...
	case MPEG_FORMAT_VCD_NSR :
		...
		{
				error_exit_msg("VCD man only have max. 1 audio and 1 video stream");
		}
...
{
	msgTitle = "OutputStream::OutputMultiplex";
	using namespace std;
	...
	*/
	...
	for(;;)
	{
		...
		*/
		...
		case start_segment :
			info_Message( "New sequence commences..." );
			SetPosAndSCR(0);
			MuxStatus( LOG_INFO );
			...
			*/
            ...
            //for( str = estreams->begin(); str < estreams->end(); ++str )
            //{
            //mjpeg_info("STREAM %02x: SCR=%ld mux=%d reqDTS=%ld", 
            //(*str)->stream_id,
            //current_SCR /300,
			...
			*/
			...
			if( psstrm->FileLimReached() )
			{
				...
				else 
				{
					if( master->NextAUType() == IFRAME)
					{
						...
						debug_Message("Running out to (raw) PTS %ld SCR=%ld", 
			...
			else if( master != 0 && master->EndSeq() )
			{
				if(  split_at_seq_end && master->Lookahead( ) != 0 )
				{
					...
					{
						error_exit_msg( "Sequence split detected %d but no following sequence found...", master->NextAUType());
					}
						
					runout_PTS = master->NextRequiredPTS();
                    debug_Message("Running out to %ld SCR=%ld", 
                                runout_PTS/300, 
                                current_SCR/300 );
                    MuxStatus( LOG_INFO );
					running_out = true;
...
*/
		...
		if( despatch )
		{
			...
			video_first = false;
			if( (current_SCR >=  earliest) && (underrun_ignore == 0) && (opt_ignore_underrun == false) )
			{
				warn_Message( "Stream %02x: data will arrive too late sent(SCR)=%ld required(DTS)=%d", 
							despatch->stream_id, 
							current_SCR/300, 
							earliest/300 );
				MuxStatus( LOG_WARN );
				...
				{
					error_exit_msg("Too many frame drops -exiting" );
				}
			}
            if( despatch->nsec > 50 &&
		...
		while( str < estreams->end() )
		{
			...
			{
				info_Message( "STREAM %02x completed @ %d.", (*str)->stream_id, (*str)->au->dorder );
				MuxStatus( LOG_DEBUG );
				(*pcomp) = true;
			}
			++str;
	...
	OutputSuffix( );
	psstrm->Close();
	info_Message( "Multiplex completion at SCR=%ld.", current_SCR/300);
	MuxStatus( LOG_INFO );
	for( str = estreams->begin(); str < estreams->end(); ++str )
	{
		(*str)->Close();
        if( (*str)->nsec <= 50 )
            info_Message( "BUFFERING stream too short for useful statistics");
        else
            info_Message( "BUFFERING min %d Buf max %d",
                        (*str)->BufferMin(),
                        (*str)->BufferMax() 
                );
	}
    ...
    if( underruns> 0 )
	{
		error_exit_msg( "MUX STATUS: Frame data under-runs detected!" );
	}
	else
	{
		info_Message( "MUX STATUS: no under-runs detected.");
	}
}
...
void OutputStream::OutputDVDPriv2 (	)
{
    ...
    uint8_t *index;
    uint8_t *sector_buf=(uint8_t*)(_alloca(sizeof(uint8_t) * sector_size));
    unsigned int tozero;
