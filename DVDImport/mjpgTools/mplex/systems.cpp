// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////// //
// systems.cpp: implementation of the PS_Stream class, part of the mjpgTools Library									 //
// --------------------------------------------------------------------------------------------------------------------- //
...
{
	msgTitle = "PS_Stream::Init";
	max_segment_size = max_seg_size;
	strncpy( filename_pat, name_pat, _MAX_PATH );
	_snprintf( cur_filename, _MAX_PATH, filename_pat, segment_num );
	strm = fopen( cur_filename, "wb" );
	if( strm == NULL )
	{
		error_exit_msg( "Could not open for writing: %s", cur_filename );
	}
}
...
PS_Stream::NextFile( )
{
	msgTitle = "PS_Stream::NextFile";
    char* prev_filename=(char*)(_alloca(sizeof(char) * (strlen(cur_filename) +1)));
	fclose(strm);
	++segment_num;
...

{
    ...
    *(index++) = type;	
    ...
    { 
		msgTitle = "PS_Stream::CreateSector";
		error_exit_msg("Packet size calculation error %d S%d P%d B%d %d %d!",
               timestamps,
    ...
    if( type == PRIVATE_STR_1 )
    {
        unsigned int syncwords_found=0;
    ...
    *(index++) = static_cast<uint8_t>(PACK_START & 0x000000ff);
    ...
    if (mpeg_version == 2)
    {
        /* Annoying: MPEG-2's SCR pack header time is different from
           all the rest... */
		uint32_t reserved_byte = RESERVED_BYTE;
...
{
	msgTitle = "PS_Stream::RawWrite";
	size_t wlen;
    if( (wlen=fwrite( buf, sizeof(uint8_t), len, strm )) != len )
    {
		err_Message("Write Failure.\nbufLength = %ld, written lenght = %ld", len, wlen);
        error_exit_msg( "Failed write: %s", cur_filename );
    }
}
