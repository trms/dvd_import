// ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////// //
// interact.cpp, part of the mjpgTools Library																			 //
// --------------------------------------------------------------------------------------------------------------------- //
	...
	)
{
	...
	for( i = 0; argv[i] != NULL; ++i )
    {
        ...
        {
            sprintf(&s[strlen(s)], "\nFile <%s> looks like an MPEG Audio stream." ,argv[i]);
            bs->undochanges( undo);
        ...
        {
            sprintf(&s[strlen(s)], "\nFile <%s> looks like an AC3 Audio stream.", argv[i]);
            bs->undochanges( undo);
        ...
        {
            sprintf(&s[strlen(s)], "\nFile <%s> looks like an MPEG Video stream.", argv[i]);
            bs->undochanges( undo);
        ...
        bs->close();
        delete bs;
        sprintf(&s[strlen(s)], "\n***File <%s> is unrecogniseable!***", argv[i]);
    }
    
    if( bad_file )
    {
        error_exit_msg( "%s\n\nUnrecogniseable file(s) detected... exiting.", s);
    }
	else
		info_Message(s);
}
